...
export const resolvers = {
  ...
  Query: {
    holochainInstances: async (parent, args, context, info) => {
      ...
      hcInstances = hcInstances.map(d => {
        return { ...d, __typename: "HolochainInstance" };
      });
    ...
    allPtPromises: async (parent, args, context, info) => {
      ...
      try {
        ...
        let res = await context.hcWs.callZome(
  ...
  Mutation: {
    ...
    createPtPromise: async (_, { ptPromiseInput }, context) => {
      ...
      let result = await context.hcWs.callZome(
        conductorConfig.instanceId,
        conductorConfig.zomeName,
        "create_pt_promise",
        { entry: ptPromiseInput }
      );
